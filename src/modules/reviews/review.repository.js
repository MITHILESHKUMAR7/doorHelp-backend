...
async function getAggregateForService(serviceId) {
  ...
  return result ? { ratingAvg: Math.round(result.avg * 10) / 10, ratingCount: result.count } : { ratingAvg: 0, ratingCount: 0 };
}

function findAllForAdmin({ isVisible, page, limit }) {
  const filter = {};
  if (isVisible !== undefined) filter.isVisible = isVisible;
  const skip = (page - 1) * limit;
  return Promise.all([
    Review.find(filter).populate('user', 'fullName').populate('service', 'title').sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
    Review.countDocuments(filter),
  ]);
}
